	                    ...
	                    <div class="col-sm-8">
		                    <div class="pipeline white lined-primary shadow">
			                    ...
			                    <div class="b-t padded-v-big">
				                    <?php echo getEduAppGTLang('file');?>: <a class="btn btn-rounded btn-sm btn-primary text-white" href="<?php echo base_url() . 'parents/viewFile/' . $row['file_name']; ?>"><i class="os-icon picons-thin-icon-thin-0042_attachment"></i> <?php echo $row['file_name'];?></a>
			                    </div>
			                    <?php endif;?>
		                        ...
		                        <div class="table-responsive">
		                            <table class="table table-lightbor table-lightfont">
			                            ...
			                            <tr>
				                            ...
				                            <td>
				                                <?php if($query->num_rows() <= 0):?>
				  		                        <a class="btn nc btn-rounded btn-sm btn-danger text-white"><?php echo getEduAppGTLang('no_delivered');?></a>
					                            <?php endif;?>
					                            <?php if($query->num_rows() > 0):?>
				  		                        <a class="btn nc btn-rounded btn-sm btn-success text-white"><?php echo getEduAppGTLang('submitted_for_review');?></a>
					                            <?php endif;?>  
				                            </td>
			                            </tr>
			                            <tr>
				                            ...
				                            <td>
				                                <?php if($query->num_rows() <= 0):?>
				                                <a class="btn btn-rounded btn-sm btn-danger text-white"><?php echo getEduAppGTLang('unrated');?></a>
				                                <?php endif;?>
				                                <?php if($query->num_rows() > 0):?>
				                                <a class="btn btn-rounded btn-sm btn-primary text-white"><?php $mark =$this->db->get_where('deliveries', array('homework_code' => $homework_code, 'student_id' => $student_id))->row()->mark; if($mark > 0) echo $mark; else echo getEduAppGTLang('waiting_mark');?></a>
				                                <?php endif;?>
				                            </td>
			                            </tr>
			                            <tr>
