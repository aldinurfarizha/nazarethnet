...
<?php 
  $posts = $this->db->get_where('team_conference' , array('team_conference_id' => base64_decode($team_conference_id), 'year' => $running_year))->result_array();
...
<!DOCTYPE html>
    ...
    <body>
        <a href="<?php echo base_url();?>teacher/team_conferences/"><i class="picons-thin-icon-thin-0131_arrow_back_undo"></i></a>
        <style type="text/css">
            body {
                padding-top: 50px;
            }
        ...
        <nav id="nav-tool" class="navbar navbar-inverse navbar-fixed-top">
            <div class="container">
                <div class="navbar-header">
                    <h4><i class="fab fa-chromecast"></i> Team live: <?php echo $row['title']?></h4>
                </div>
                <div class="navbar-form navbar-right">
                    <h5><i class="far fa-user-circle" style=""></i> Tu: <?php echo $this->crud->get_name('teacher', $this->session->userdata('login_user_id'));?></h5>
                </div>
            </div>
        </nav>
        ...
        <script type="text/javascript">
            ...
            var meetConfig = {
                ...
                meetingNumber: "<?php echo $row['zoom_meeting_id']?>",
                userName: "<?php echo $this->crud->get_name('teacher', $this->session->userdata('login_user_id'));?>",
                passWord: "<?php echo $row['zoom_meeting_password']?>",
                leaveUrl: "<?php echo base_url();?>teacher/team_conferences/",
                userEmail: "<?php echo $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->email;?>",
