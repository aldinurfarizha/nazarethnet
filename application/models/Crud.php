...
class Crud extends School 
{
    ...
    function driveSettings(){
        $data['description'] = strip_tags($this->input->post('delete_drive'));
        $this->db->where('type' , 'delete_drive');
        $this->db->update('settings' , $data);
    }
    
    
    function calculate_average($online_exam_id, $student_id){
    ...
    function getSubjects($class_id, $section_id)
    {
    ...
    function timetable($hour = '', $row = '', $class_id = '', $section_id = '') 
    {
        ...
        $this->db->select('CONCAT(t.tbl_row, "_", t.tbl_col) as posicion, t.class_routine_id, t.subject_id, s.name,s.color', false);
        $this->db->join('class_routine AS t', 't.subject_id = s.subject_id');
        $this->db->where('t.class_id', $class_id);
        $this->db->where('t.section_id', $section_id);
        $rss =  $this->db->get('subject AS s')->result_array(); 
        $rs = [];
        foreach ($rss as $data) 
        {
            $values = array_values($data);
            $rs[$data['posicion']] = $values;
        }
        echo '<tr style="background:#fff">';
        echo '<td class="dark">' . $hour . '</td>';
        for ($col = 1; $col <= $days; $col++) 
        {
            ...
            if (array_key_exists($pos, $rs))
            {
                ...
                $name = $elements[3];
                $class = substr($id, 0, 2);
                echo "<div id=\"$id\" class=\"redips-drag $class\" style='border: 2px solid #$elements[4];color:#$elements[4];font-weight: bold;padding:10px;background-color:#fff;'>$name</div>";
            }
            echo '</td>';
        }
        echo "</tr>\n";
    ...
    function teacher_timetable($hour = '', $row = '', $teacher_id = '') 
    {
        $sundays = $this->db->get_where('academic_settings' , array('type' =>'routine'))->row()->description;
        if($sundays == 1)
        {
        ...
        for ($col = 1; $col <= $days; $col++) 
        {
            ...
            if (array_key_exists($pos, $rs))
            {
                ...
                $cl_id = $this->db->get_where('subject', array('subject_id' => $elements[2]))->row()->class_id; 
                $sc_id = $this->db->get_where('subject', array('subject_id' => $elements[2]))->row()->section_id; 
                $ncl   = $this->db->get_where('class', array('class_id' => $cl_id))->row()->name; 
                $nsc   = $this->db->get_where('section', array('section_id' => $sc_id))->row()->name; 
                echo "<div id=\"$id\" class=\"redips-drag $class\" style='border: 2px solid #$cxol;color:#$cxol;font-weight: bold;padding:10px;background-color:#fff;'>$name - $ncl - $nsc</div>";    
            }
            echo '</td>';
        }
        echo "</tr>\n";
    ...
    function getCertificatePDF($onlineExamId){
        $data = array(
            'student_id' => $this->session->userdata('login_user_id'),
            'onlineExamId' => base64_decode($onlineExamId)
        );
        $today = date('d-m-Y_h:i:s');
        if($this->db->get_where('academic_settings', array('type' => 'certificate_style'))->row()->description == 1){
            $html = $this->load->view('backend/certificates/template-1.php',$data,TRUE); 
            $stylesheet = file_get_contents(base_url().'public/certificates/template-1.css');   
        }else{
            $html = $this->load->view('backend/certificates/template-2.php',$data,TRUE); 
            $stylesheet = file_get_contents(base_url().'public/certificates/template-2.css');
        }
        $pdfFilePath = "certificate-".$today.".pdf";
    ...
    function attendanceQR($student_id, $subject_id)
    {
        $this->load->library('ciqrcode');
        $params['data'] = base_url().'verification/takeAttendance?data='.base64_encode($student_id.'-'.$subject_id);
        $params['level'] = 'H';
        $params['size'] = 20;
        $params['savename'] = FCPATH . "public/uploads/attendance/qr_".base64_encode($student_id.$subject_id).".png";
        $this->ciqrcode->generate($params);
        return base_url()."public/uploads/attendance/qr_".base64_encode($student_id.$subject_id).".png";
    }

    function setAttendance($student_id, $subject_id)
    {
        ...
        $data['year']       = date('Y');
        $data['timestamp']  = $date;
        $data['section_id'] = $this->db->get_where('enroll', array('student_id' => $student_id))->row()->section_id;
        ...
        if($query->num_rows() == 0) 
        {
            $attn_data['class_id']   = $this->db->get_where('enroll', array('student_id' => $student_id))->row()->class_id;
            $attn_data['subject_id'] = $data['subject_id'];
            $attn_data['year']       = date('Y');
            $attn_data['timestamp']  = $data['timestamp'];
            $attn_data['section_id'] = $this->db->get_where('enroll', array('student_id' => $student_id))->row()->section_id;
            $attn_data['student_id'] = $student_id;
            $attn_data['time']       = date('H:i:s');
    ...
    {
        $md = md5(date('d/m/Y h:i:s'));
        $data['description']  = $this->input->post('certificate_title');
        $this->db->where('type' , 'certificate_title');
        $this->db->update('academic_settings' , $data);

        $data['description']  = $this->input->post('certificate_subtitle');
        $this->db->where('type' , 'certificate_subtitle');
        $this->db->update('academic_settings' , $data);
        
        $data['description']  = $this->input->post('certificate_text');
        $this->db->where('type' , 'certificate_text');
        $this->db->update('academic_settings' , $data);
        
        $data['description'] = $this->input->post('certificate_footer');
        $this->db->where('type' , 'certificate_footer');
        $this->db->update('academic_settings' , $data);
        ...
        $data['description'] = $this->input->post('certificate_style');
        $this->db->where('type' , 'certificate_style');
        $this->db->update('academic_settings' , $data);
        
        $data['description'] = $this->input->post('ceo_name');
        $this->db->where('type' , 'ceo_name');
        $this->db->update('academic_settings' , $data);
        
        if($_FILES['ceo_signature']['size'] > 0){
            $data['description'] = $md.$_FILES["ceo_signature"]["name"];
            $this->db->where('type' , 'ceo_signature');
            $this->db->update('academic_settings' , $data);   
        }
        move_uploaded_file($_FILES["ceo_signature"]["tmp_name"], "public/uploads/" . $md.$_FILES["ceo_signature"]["name"]);
    }
    
    function getStudentQR($student_id)
    {
        $this->load->library('ciqrcode');
        $params['data']     = base_url().'verification/qr?data='.base64_encode($student_id);
        $params['level']    = 'H';
        $params['size']     = 20;
        $params['savename'] = FCPATH . "public/uploads/qrcodes/qr_".base64_encode($student_id).".png";
        $this->ciqrcode->generate($params);
        return base_url()."public/uploads/qrcodes/qr_".base64_encode($student_id).".png";
    }

    function getFront($type) {
        $query = $this->db->get_where('frontend', array('type' => $type));
        return $query->row()->description;
    }

    function contact(){
        $doc = new DOMDocument();
        $doc->loadHTML($this->input->post('map_contact'));
        $xpath = new DOMXPath($doc);
        $src = $xpath->evaluate("string(//iframe/@src)");
        $data['description'] = $src;
        $this->db->where('type' , 'map_contact');
        $this->db->update('frontend' , $data);
    }

    function recaptcha(){
        $data['description'] = strip_tags($this->input->post('site_key'));
        $this->db->where('type' , 'site_key');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('secret_key'));
        $this->db->where('type' , 'secret_key');
        $this->db->update('frontend' , $data);
    }
    ...
    function footer(){
        ...
        $data['description'] = strip_tags($this->input->post('footer_text'));
        $this->db->where('type' , 'footer_text');
        $this->db->update('frontend' , $data);

        if($_FILES['footer_logo']['size'] > 0)
        {
            $data['description'] = $img.str_replace(' ', '', $_FILES['footer_logo']['name']);
            $this->db->where('type' , 'footer_logo');
            $this->db->update('frontend' , $data);
            move_uploaded_file($_FILES['footer_logo']['tmp_name'], 'public/uploads/frontend/' . $img.str_replace(' ', '', $_FILES['footer_logo']['name']));
        }
    }
    ...
    function about(){
        ...
        $data['description'] = strip_tags($this->input->post('about_title'));
        $this->db->where('type' , 'about_title');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('about_subtitle'));
        $this->db->where('type' , 'about_subtitle');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('about_text_button'));
        $this->db->where('type' , 'about_text_button');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('about_redirect_button'));
        $this->db->where('type' , 'about_redirect_button');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('about_why'));
        $this->db->where('type' , 'widget_title');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_why_subtitle'));
        $this->db->where('type' , 'widget_subtitle');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_mission'));
        $this->db->where('type' , 'about_mission');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_mission_description'));
        $this->db->where('type' , 'about_mission_description');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_vission'));
        $this->db->where('type' , 'about_vission');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_vission_description'));
        $this->db->where('type' , 'about_vission_description');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_bottom_title'));
        $this->db->where('type' , 'about_bottom_title');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_bottom_subtitle'));
        $this->db->where('type' , 'about_bottom_subtitle');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_intro_text'));
        $this->db->where('type' , 'about_intro_text');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_title_1'));
        $this->db->where('type' , 'about_title_1');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('about_description_1'));
        $this->db->where('type' , 'about_description_1');
        $this->db->update('frontend' , $data);
        ...
        {
            $data['description'] = $img.str_replace(' ', '', $_FILES['about_image']['name']);
            $this->db->where('type' , 'about_image');
            $this->db->update('frontend' , $data);
            move_uploaded_file($_FILES['about_image']['tmp_name'], 'public/uploads/frontend/' . $img.str_replace(' ', '', $_FILES['about_image']['name']));
        }

        if($_FILES['about_image_widget']['size'] > 0)
        {
            $data['description'] = $img.str_replace(' ', '', $_FILES['about_image_widget']['name']);
            $this->db->where('type' , 'about_image_widget');
            $this->db->update('frontend' , $data);
            move_uploaded_file($_FILES['about_image_widget']['tmp_name'], 'public/uploads/frontend/' . $img.str_replace(' ', '', $_FILES['about_image_widget']['name']));
        }
    }

    function whyUS(){
        $data['description'] = strip_tags($this->input->post('why_us_title'));
        $this->db->where('type' , 'why_us_title');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('why_us_subtitle'));
        $this->db->where('type' , 'why_us_subtitle');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('why_us_card_1_title'));
        $this->db->where('type' , 'why_us_card_1_title');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('why_us_card_1_description'));
        $this->db->where('type' , 'why_us_card_1_description');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('why_us_card_2_title'));
        $this->db->where('type' , 'why_us_card_2_title');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('why_us_card_2_description'));
        $this->db->where('type' , 'why_us_card_2_description');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('why_us_card_3_title'));
        $this->db->where('type' , 'why_us_card_3_title');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('why_us_card_3_description'));
        $this->db->where('type' , 'why_us_card_3_description');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('why_us_bottom_title'));
        $this->db->where('type' , 'why_us_bottom_title');
        $this->db->update('frontend' , $data);

        $data['description'] = strip_tags($this->input->post('why_us_bottom_subtitle'));
        $this->db->where('type' , 'why_us_bottom_subtitle');
        $this->db->update('frontend' , $data);
    }

    function second(){
        $img = md5(date('d/m/Y H:i:s')); 
        $data['description'] = strip_tags($this->input->post('second_intro'));
        $this->db->where('type' , 'second_intro');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('second_title'));
        $this->db->where('type' , 'second_title');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('second_description'));
        $this->db->where('type' , 'second_description');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('second_text_button'));
        $this->db->where('type' , 'second_text_button');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('second_redirect_button'));
        $this->db->where('type' , 'second_redirect_button');
        $this->db->update('frontend' , $data);

        if($_FILES['second_image']['size'] > 0)
        {
            $data['description'] = $img.str_replace(' ', '', $_FILES['second_image']['name']);
            $this->db->where('type' , 'second_image');
            $this->db->update('frontend' , $data);
            move_uploaded_file($_FILES['second_image']['tmp_name'], 'public/uploads/frontend/' . $img.str_replace(' ', '', $_FILES['second_image']['name']));
        }
    }

    function principal(){
        $img = md5(date('d/m/Y H:i:s')); 
        $data['description'] = strip_tags($this->input->post('principal_intro'));
        $this->db->where('type' , 'principal_intro');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('principal_title'));
        $this->db->where('type' , 'principal_title');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('principal_description'));
        $this->db->where('type' , 'principal_description');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('principal_text_button'));
        $this->db->where('type' , 'principal_text_button');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('principal_redirect_button'));
        $this->db->where('type' , 'principal_redirect_button');
        $this->db->update('frontend' , $data);

        if($_FILES['principal_image']['size'] > 0)
        {
            $data['description'] = $img.str_replace(' ', '', $_FILES['principal_image']['name']);
            $this->db->where('type' , 'principal_image');
            $this->db->update('frontend' , $data);
            move_uploaded_file($_FILES['principal_image']['tmp_name'], 'public/uploads/frontend/' . $img.str_replace(' ', '', $_FILES['principal_image']['name']));
        }
    }

    function homeWidget(){
        $img = md5(date('d/m/Y H:i:s')); 
        $data['description'] = strip_tags($this->input->post('home_title'));
        $this->db->where('type' , 'home_title');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('home_subtitle'));
        $this->db->where('type' , 'home_subtitle');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('home_text_button_1'));
        $this->db->where('type' , 'home_text_button_1');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('home_redirect_button_1'));
        $this->db->where('type' , 'home_redirect_button_1');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('home_text_button_2'));
        $this->db->where('type' , 'home_text_button_2');
        $this->db->update('frontend' , $data);
        
        $data['description'] = strip_tags($this->input->post('home_redirect_button_2'));
        $this->db->where('type' , 'home_redirect_button_2');
        $this->db->update('frontend' , $data);

        if($_FILES['home_image']['size'] > 0)
        {
            $data['description'] = $img.str_replace(' ', '', $_FILES['home_image']['name']);
            $this->db->where('type' , 'home_image');
            $this->db->update('frontend' , $data);
            move_uploaded_file($_FILES['home_image']['tmp_name'], 'public/uploads/frontend/' . $img.str_replace(' ', '', $_FILES['home_image']['name']));
        }
    }
    ...
    function deleteImage($imgID){
        ...
        $name = $this->db->get_where('gallery', array('gallery_id' => $imgID))->row()->name;
        unlink('public/uploads/gallery/'.$name);
        $this->db->where('gallery_id', $imgID);
    ...
    function create_vimeo() 
    {
        ...
        $data['embed']                  = substr(parse_url($this->input->post('embedvimeo'), PHP_URL_PATH), 1);
    ...
    function create_vimeo_dash($codd) 
    {
    ...
    function create_video_dash($codd) 
    {
        $info = base64_decode($codd);
        $ex = explode('-', $info);
        $img                            = md5(date('d/m/Y H:i:s')); 
    ...
    function updateCalendarDate()
    {
        ...
        {
            $id = html_escape($_POST['Event'][0]);
            $start = html_escape($_POST['Event'][1]);
            $end = html_escape($_POST['Event'][2]);
    ...
    function createCalendarEvent()
    {
        ...
        {
            $title = html_escape($_POST['title']);
            $start = html_escape($_POST['start']);
            $end   = html_escape($_POST['end']);
        	$color = html_escape($_POST['color']);
    ...
    {
        if($this->input->post('frontend') == 1){
            $replace = read_file('./application/config/routes.php');
            $replace = str_replace('login','home',$replace);
            write_file('./application/config/routes.php', $replace);
        }else{
            $replace = read_file('./application/config/routes.php');
            $replace = str_replace('home','login',$replace);
            write_file('./application/config/routes.php', $replace);
        }
        $data['description'] = strip_tags($this->input->post('frontend'));
    ...
    function fetchStudents($section_id)
    {
        $html = '';
    ...
    function createBus()
    {
        ...
        $data['driver_name']       = html_escape($this->input->post('driver_name'));
        $data['driver_phone']      = html_escape($this->input->post('driver_phone'));
    ...
    {
        $data['title']               =   html_escape($this->input->post('title'));
        $data['expense_category_id'] =   html_escape($this->input->post('expense_category_id'));
        $data['description']         =   html_escape($this->input->post('description'));
        $data['payment_type']        =   'expense';
        $data['method']              =   html_escape($this->input->post('method'));
    ...
    function createExpense()
    {
        ...
        $data['amount']              =   html_escape($this->input->post('amount'));
        $data['month']               =   date('m');
        $data['timestamp']           =   html_escape($this->input->post('timestamp'));
        $data['month']               =   date('M');
        $data['year']                =   $this->runningYear;
    ...
    function twilio_api($message = "", $reciever = "") 
    {
        ...
        $twilio_number = $this->db->get_where('settings', array('type' => 'registered_phone'))->row()->description;
    ...
    {
        if(html_escape($_POST['c']) != "")
        {
            $credential = array('username' => html_escape($_POST['c']));
            $query = $this->db->get_where('admin', $credential);
            if ($query->num_rows() > 0) 
            {
                return 'success';
    ...
    {
        $qry = $this->db->get_where('student_gamification', array('activity_type' => 'live', 'activity_id' => $liveId, 'student_id' => $this->session->userdata('login_user_id')));
        if($qry->num_rows() == 0){
            $exp = $this->db->get_where('zoom', array('zoom_id' => $liveId))->row()->exp;
            $class_id = $this->db->get_where('zoom', array('zoom_id' => $liveId))->row()->class_id;
            $section_id = $this->db->get_where('zoom', array('zoom_id' => $liveId))->row()->section_id;
            $subject_id = $this->db->get_where('zoom', array('zoom_id' => $liveId))->row()->subject_id;
            $verify = $this->db->get_where('subject', array('subject_id' => $subject_id))->row()->live_class;
            ...
            $gam_insert['activity_id']   = $liveId;
            $gam_insert['activity_type'] = 'live';
            if($verify == 1){
    ...
    function getExpImg($subject_id, $student_id, $class_id, $section_id){
        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $currentExp = $this->getExp($subject_id, $student_id, $class_id, $section_id);
        $returnLevel = 0;
        ...
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 1) && $currentExp < $this->getLevelPoint($subject_id, $class_id, $section_id, 2)){
            $returnLevel = 1;
        }
        //For Level 2.
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 2) && $currentExp < $this->getLevelPoint($subject_id, $class_id, $section_id, 3)){
            $returnLevel = 2;
        }
        //For Level 3.
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 3) && $currentExp < $this->getLevelPoint($subject_id, $class_id, $section_id, 4)){
            $returnLevel = 3;
        }
        //For Level 4.
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 4) && $currentExp < $this->getLevelPoint($subject_id, $class_id, $section_id, 5)){
            $returnLevel = 4;
        }
        //For Level 5.
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 5) && $currentExp < $this->getLevelPoint($subject_id, $class_id, $section_id, 6)){
            $returnLevel = 5;
        }
        //For Level 6.
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 6) && $currentExp < $this->getLevelPoint($subject_id, $class_id, $section_id, 7)){
            $returnLevel = 6;
        }
        //For Level 7.
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 7) && $currentExp < $this->getLevelPoint($subject_id, $class_id, $section_id, 8)){
            $returnLevel = 7;
        }
        //For Level 8.
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 8) && $currentExp < $this->getLevelPoint($subject_id, $class_id, $section_id, 9)){
            $returnLevel = 8;
        }
        //For Level 9.
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 9) && $currentExp < $this->getLevelPoint($subject_id, $class_id, $section_id, 10)){
            $returnLevel = 9;
        }
        //For Level 10.
        elseif($currentExp >= $this->getLevelPoint($subject_id, $class_id, $section_id, 10) && $this->getLevelPoint($subject_id, $class_id, $section_id, 10) != 0){
            $returnLevel = 10;
        }
        return $returnLevel;
    }
    
    function getLevelPoint($subject_id, $class_id, $section_id, $level){
        return $this->db->get_where('gamification', array('class_id' => $class_id, 'section_id' => $section_id, 'subject_id' => $subject_id, 'level' => $level))->row()->point;
    }
    
    function getExp($subject_id, $student_id, $class_id, $section_id){
        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $query = $this->db->get_where('student_gamification', array('subject_id' => $subject_id, 'student_id' => $student_id, 'class_id' => $class_id, 'section_id' => $section_id, 'year' => $year))->result_array();
        $totalExp = 0;
        foreach($query as $x){
            $totalExp += $x['exp'];
        }
    ...
    function count_unread_messages() 
{
    ...
    {
        $year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;
        $qry = $this->db->get_where('student_gamification', array('activity_type' => 'forum', 'activity_id' => $post_code, 'student_id' => $this->session->userdata('login_user_id')));
        if($qry->num_rows() == 0){
            $exp = $this->db->get_where('forum', array('post_code' => $post_code))->row()->exp;
            $class_id = $this->db->get_where('forum', array('post_code' => $post_code))->row()->class_id;
            $section_id = $this->db->get_where('forum', array('post_code' => $post_code))->row()->section_id;
            $subject_id = $this->db->get_where('forum', array('post_code' => $post_code))->row()->subject_id;
            $verify = $this->db->get_where('subject', array('subject_id' => $subject_id))->row()->live_class;
            $gam_insert['student_id']    = $this->session->userdata('login_user_id');
            $gam_insert['subject_id']    = $subject_id;
            $gam_insert['date']          = date('d/m/Y');
            ...
            $gam_insert['class_id']      = $class_id;
            $gam_insert['section_id']    = $section_id;
            $gam_insert['activity_id']   = $post_code;
            $gam_insert['activity_type'] = 'forum';
    ...
    function delete_homework($homework_code) {
     ...
     function delete_post($post_code) {
        $fileID = $this->db->get_where('forum', array('post_code' => $post_code))->row()->file_name;
        if($fileID != ''){
            $this->drive_model->deleteFile($fileID);
        }
        $this->db->where('post_code', $post_code);
    ...
    function income()
    {
        ...
        {
            $total = 0; 
            $income = $this->db->get_where('payment', array('month' => $months[$i], 'payment_type' => 'income'))->result_array();
    ...
    function expense()
    {
        ...
        for($i = 0; $i < 12; $i++)
        {
            $expese = $this->db->get_where('payment', array('month' => $months[$i],'payment_type' => 'expense'))->result_array();
            $total = 0;
    ...
    function get_image_url($type = '', $id = '') 
    {
        ...
        $name = trim(strtoupper($this->db->get_where(''.$type.'',array($type."_id" => $id))->row()->first_name));
    ...
    function create_news_dash($codd) 
    {
        $info = base64_decode($codd);
        $ex = explode('-', $info);
    ...
    function updateDrive(){
        $data['description'] = strip_tags($this->input->post('ClientSecret'));
        $this->db->where('type' , 'ClientSecret');
        $this->db->update('settings' , $data);
    ...
    {
        $data['description']  = html_escape($this->input->post('report_teacher'));
        $this->db->where('type' , 'students_reports');
        $this->db->update('settings' , $data);

        $data['description']  = html_escape($this->input->post('minium_mark'));
        ...
        $data['description'] = $this->input->post('start_1');
        $this->db->where('type' , 'start_1');
        $this->db->update('settings' , $data);
        
        $data['description'] = $this->input->post('final_1');
        $this->db->where('type' , 'final_1');
        $this->db->update('settings' , $data);
        
        $data['description'] = $this->input->post('class_duration');
        $this->db->where('type' , 'class_duration');
        $this->db->update('settings' , $data);
        
        $data['description'] = $this->input->post('start_2');
        $this->db->where('type' , 'start_2');
        $this->db->update('settings' , $data);
        
        $data['description'] = $this->input->post('final_2');
        $this->db->where('type' , 'final_2');
        $this->db->update('settings' , $data);
    }
    
    function setPermissions()
    {
        $data['permissions'] = html_escape($this->input->post('messages'));
        $this->db->where('type' , 'messages');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('admins'));
        $this->db->where('type' , 'admins');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('parents'));
        $this->db->where('type' , 'parents');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('teachers'));
        $this->db->where('type' , 'teachers');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('students'));
        $this->db->where('type' , 'students');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('accountants'));
        $this->db->where('type' , 'accountants');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('librarians'));
        $this->db->where('type' , 'librarians');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('library'));
        $this->db->where('type' , 'library');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('academic'));
        $this->db->where('type' , 'academic');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('attendance'));
        $this->db->where('type' , 'attendance');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('calendar'));
        $this->db->where('type' , 'calendar');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('files'));
        $this->db->where('type' , 'files');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('polls'));
        $this->db->where('type' , 'polls');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('notifications'));
        $this->db->where('type' , 'notifications');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('admissions'));
        $this->db->where('type' , 'admissions');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('behavior'));
        $this->db->where('type' , 'behavior');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('news'));
        $this->db->where('type' , 'news');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('school_bus'));
        $this->db->where('type' , 'school_bus');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('classrooms'));
        $this->db->where('type' , 'classrooms');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('accounting'));
        $this->db->where('type' , 'accounting');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('schedules'));
        $this->db->where('type' , 'schedules');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('system_reports'));
        $this->db->where('type' , 'system_reports');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('academic_settings'));
        $this->db->where('type' , 'academic_settings');
        $this->db->update('account_role' , $data);
        
        $data['permissions'] = html_escape($this->input->post('settings'));
    ...
    function attendanceSelector()
    {
        ...
        $data['year']       = html_escape($this->input->post('year'));
        $originalDate       = html_escape($this->input->post('timestamp'));
        $newDate            = date("d-m-Y", strtotime($originalDate));
        $data['timestamp']  = strtotime($newDate);
        $data['section_id'] = $this->input->post('section_id');
            $query = $this->db->get_where('attendance' ,array(
    ...
    function sendSMS()
    {
        ...
        $receiver   = html_escape($this->input->post('receiver'));
        if($receiver == 'student'){
    ...
    function smsStatus()
    {
        $data['description'] = html_escape($this->input->post('sms_status'));
    ...
    function services()
    {
        $data['description'] = html_escape($this->input->post('absences'));
        $this->db->where('type' , 'absences');
        $this->db->update('settings' , $data);

        $data['description'] = html_escape($this->input->post('students_reports'));
        $this->db->where('type' , 'students_reports');
        $this->db->update('settings' , $data);

        $data['description'] = html_escape($this->input->post('p_new_invoice'));
        $this->db->where('type' , 'p_new_invoice');
        $this->db->update('settings' , $data);

        $data['description'] = html_escape($this->input->post('new_homework'));
        $this->db->where('type' , 'new_homework');
        $this->db->update('settings' , $data);

        $data['description'] = html_escape($this->input->post('s_new_invoice'));
    ...
    {
        $data[$lang] = html_escape($this->input->post('phrase'));
        $this->db->where('phrase' , html_escape($this->input->post('phrase_key')));
    ...
    {
        $data['poll_code'] = html_escape($this->input->post('poll_code'));
